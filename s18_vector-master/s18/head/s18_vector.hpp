    ...
    namespace s18 {
        ...
        public:
            std::tuple<uint64_t, uint64_t, uint64_t> metadata(void) const {
                // Return leading 1s, case as intand len w / o leading ones
        ...
        private:
            ...
            uint64_t find_block_nth(int_vector<32>::const_iterator const begin, int_vector<32>::const_iterator const end, uint64_t target_accum) const {
                ...
                for (; std::distance(gaps, end) > 0; ++gaps) {
                    ...
                    //     break;
                    // default:
                    //     switch (value & MASK_HEADER5) {
                    //     case CASE16:
                    //         _case = 15;
                    //         leading_1s = (value & MASK_BODY5);
                    //         len = 0;
                    //         break;
                    //     case CASE17:
                    //         _case = 16;
                    ...
                    //     wi = (w.value & word::DECODER_MASK[_case][i]) >> word::DECODER_SHIFT[_case][i];
                    //     accum += wi;
                    //     if (accum >= target_accum || wi == 0) break;
                    // }
                    // if (accum >= target_accum) return one_cnt;

                    // -- Version que deio los mejroes resultados en test locales --
                    uint64_t wi = 0;
                    switch (len) {
                    case 0:
                        break;
                    case 1:
                        for (uint64_t i = 0; i < 1; i++, one_cnt++) {
                            wi = (w.value & word::DECODER_MASK[_case][i]) >> word::DECODER_SHIFT[_case][i];
                            accum += wi;
                            if (accum >= target_accum || wi == 0) break;
                        }
                        ...
                        break;
                    case 2:
                        for (uint64_t i = 0; i < 2; i++, one_cnt++) {
                            wi = (w.value & word::DECODER_MASK[_case][i]) >> word::DECODER_SHIFT[_case][i];
                            accum += wi;
                            if (accum >= target_accum || wi == 0) break;
                        }
                        ...
                        break;
                    case 3:
                        for (uint64_t i = 0; i < 3; i++, one_cnt++) {
                            wi = (w.value & word::DECODER_MASK[_case][i]) >> word::DECODER_SHIFT[_case][i];
                            accum += wi;
                            if (accum >= target_accum || wi == 0) break;
                        }
                        ...
                        break;
                    case 4:
                        for (uint64_t i = 0; i < 4; i++, one_cnt++) {
                            wi = (w.value & word::DECODER_MASK[_case][i]) >> word::DECODER_SHIFT[_case][i];
                            accum += wi;
                            if (accum >= target_accum || wi == 0) break;
                        }
                        ...
                        break;
                    case 5:
                        for (uint64_t i = 0; i < 5; i++, one_cnt++) {
                            wi = (w.value & word::DECODER_MASK[_case][i]) >> word::DECODER_SHIFT[_case][i];
                            accum += wi;
                            if (accum >= target_accum || wi == 0) break;
                        }
                        ...
                        break;
                    case 7:
                        for (uint64_t i = 0; i < 7; i++, one_cnt++) {
                            wi = (w.value & word::DECODER_MASK[_case][i]) >> word::DECODER_SHIFT[_case][i];
                            accum += wi;
                            if (accum >= target_accum || wi == 0) break;
                        }
                        ...
                        break;
                    case 9:
                        for (uint64_t i = 0; i < 9; i++, one_cnt++) {
                            wi = (w.value & word::DECODER_MASK[_case][i]) >> word::DECODER_SHIFT[_case][i];
                            accum += wi;
                            if (accum >= target_accum || wi == 0) break;
                        }
                        ...
                        break;
                    case 14:
                        for (uint64_t i = 0; i < 14; i++, one_cnt++) {
                            wi = (w.value & word::DECODER_MASK[_case][i]) >> word::DECODER_SHIFT[_case][i];
                            accum += wi;
                            if (accum >= target_accum || wi == 0) break;
                        }
                    ...
                    // -- Intento mas lento que original --
                    // Cuidado, tiene un bug en alguna parte y da resultados incorrectos!
                    ...
                    //     break;
                    // case 1:
                    //     wi = (w.value & word::DECODER_MASK[_case][0]) >> word::DECODER_SHIFT[_case][0];
                    //     accum += wi;
                    //     if (wi == 0) break;
                    //     if (accum >= target_accum) return one_cnt;
                    //     one_cnt++;
                    ...
                    //     wi = (w.value & word::DECODER_MASK[_case][1]) >> word::DECODER_SHIFT[_case][1];
                    //     accum += wi;
                    //     if (wi == 0) break;
                    //     if (accum >= target_accum) return one_cnt;
                    //     one_cnt++;
                    ...
                    //     wi = (w.value & word::DECODER_MASK[_case][2]) >> word::DECODER_SHIFT[_case][2];
                    //     accum += wi;
                    //     if (wi == 0) break;
                    //     if (accum >= target_accum) return one_cnt;
                    //     one_cnt++;
                    ...
                    //     wi = (w.value & word::DECODER_MASK[_case][3]) >> word::DECODER_SHIFT[_case][3];
                    //     accum += wi;
                    //     if (wi == 0) break;
                    //     if (accum >= target_accum) return one_cnt;
                    //     one_cnt++;
                    ...
                    //     (w.value & word::DECODER_MASK[_case][4]) >> word::DECODER_SHIFT[_case][4];
                    //     accum += wi;
                    //     if (wi == 0) break;
                    //     if (accum >= target_accum) return one_cnt;
                    //     one_cnt++;
                    //     break;
                    // case 7:
                    //     (w.value & word::DECODER_MASK[_case][0]) >> word::DECODER_SHIFT[_case][0];
                    //     accum += wi;
                    //     if (wi == 0) break;
                    //     if (accum >= target_accum) return one_cnt;
                    //     one_cnt++;
                    ...
                    //     wi = (w.value & word::DECODER_MASK[_case][4]) >> word::DECODER_SHIFT[_case][4];
                    //     accum += wi;
                    //     if (wi == 0) break;
                    //     if (accum >= target_accum) return one_cnt;
                    //     one_cnt++;

                    //     wi = (w.value & word::DECODER_MASK[_case][5]) >> word::DECODER_SHIFT[_case][5];
                    //     accum += wi;
                    //     if (wi == 0) break;
                    //     if (accum >= target_accum) return one_cnt;
                    //     one_cnt++;

                    //     wi = (w.value & word::DECODER_MASK[_case][6]) >> word::DECODER_SHIFT[_case][6];
                    //     accum += wi;
                    //     if (wi == 0) break;
                    //     if (accum >= target_accum) return one_cnt;
                    //     one_cnt++;
                    ...
                    //     wi = (w.value & word::DECODER_MASK[_case][7]) >> word::DECODER_SHIFT[_case][7];
                    //     accum += wi;
                    //     if (wi == 0) break;
                    //     if (accum >= target_accum) return one_cnt;
                    //     one_cnt++;

                    //     wi = (w.value & word::DECODER_MASK[_case][8]) >> word::DECODER_SHIFT[_case][8];
                    //     accum += wi;
                    //     if (wi == 0) break;
                    //     if (accum >= target_accum) return one_cnt;
                    //     one_cnt++;
                    ...
                    //     wi = (w.value & word::DECODER_MASK[_case][9]) >> word::DECODER_SHIFT[_case][9];
                    //     accum += wi;
                    //     if (wi == 0) break;
                    //     if (accum >= target_accum) return one_cnt;
                    //     one_cnt++;

                    //     wi = (w.value & word::DECODER_MASK[_case][10]) >> word::DECODER_SHIFT[_case][10];
                    //     accum += wi;
                    //     if (wi == 0) break;
                    //     if (accum >= target_accum) return one_cnt;
                    //     one_cnt++;

                    //     wi = (w.value & word::DECODER_MASK[_case][11]) >> word::DECODER_SHIFT[_case][11];
                    //     accum += wi;
                    //     if (wi == 0) break;
                    //     if (accum >= target_accum) return one_cnt;
                    //     one_cnt++;

                    //     wi = (w.value & word::DECODER_MASK[_case][12]) >> word::DECODER_SHIFT[_case][12];
                    //     accum += wi;
                    //     if (wi == 0) break;
                    //     if (accum >= target_accum) return one_cnt;
                    //     one_cnt++;

                    //     wi = (w.value & word::DECODER_MASK[_case][13]) >> word::DECODER_SHIFT[_case][13];
                    //     accum += wi;
                    //     if (wi == 0) break;
                    //     if (accum >= target_accum) return one_cnt;
                    //     one_cnt++;
                    ...
                    // -- Unrolling con ifs, mas lento que original --
                    // uint64_t wi;
                    // if (len == 0) continue;

                    // wi = w.access_fast(0, _case);
                    // if (wi == 0) continue;
                    // accum += wi;
                    // if (accum >= target_accum) return one_cnt;
                    // one_cnt++;
                    ...
                    // wi = w.access_fast(5, _case);
                    // if (wi == 0) continue;
                    // accum += wi;
                    // if (accum >= target_accum) return one_cnt;
                    // one_cnt++;

                    // wi = w.access_fast(6, _case);
                    // if (wi == 0) continue;
                    // accum += wi;
                    // if (accum >= target_accum) return one_cnt;
                    // one_cnt++;
                    ...
                    // wi = w.access_fast(7, _case);
                    // if (wi == 0) continue;
                    // accum += wi;
                    // if (accum >= target_accum) return one_cnt;
                    // one_cnt++;

                    // wi = w.access_fast(8, _case);
                    // if (wi == 0) continue;
                    // accum += wi;
                    // if (accum >= target_accum) return one_cnt;
                    // one_cnt++;
                    ...
                    // wi = w.access_fast(9, _case);
                    // if (wi == 0) continue;
                    // accum += wi;
                    // if (accum >= target_accum) return one_cnt;
                    // one_cnt++;

                    // wi = w.access_fast(10, _case);
                    // if (wi == 0) continue;
                    // accum += wi;
                    // if (accum >= target_accum) return one_cnt;
                    // one_cnt++;

                    // wi = w.access_fast(11, _case);
                    // if (wi == 0) continue;
                    // accum += wi;
                    // if (accum >= target_accum) return one_cnt;
                    // one_cnt++;

                    // wi = w.access_fast(12, _case);
                    // if (wi == 0) continue;
                    // accum += wi;
                    // if (accum >= target_accum) return one_cnt;
                    // one_cnt++;

                    // wi = w.access_fast(13, _case);
                    // if (wi == 0) continue;
                    // accum += wi;
                    // if (accum >= target_accum) return one_cnt;
                    // one_cnt++;
        ...
        private:
            ...
            uint64_t partial_sum(int_vector<32>::const_iterator const begin, int_vector<32>::const_iterator const end, uint64_t counter) const {
                ...
                for (; std::distance(gaps, end) > 0 and counter; gaps++) {
                    ...
                    // -- Codigo original --
                    uint64_t wi = 1;
                    for (uint64_t i = 0; i < len and counter && wi != 0; i++, counter--) {
                        wi = (w.value & word::DECODER_MASK[_case][i]) >> word::DECODER_SHIFT[_case][i];
                        // if (wi == 0) break; /* Word was not full */
                        accum += wi;
                    }
                    ...
                    // -- Mas lento que original en test locales --
                    // uint64_t wi = 1;
                    // switch (len) {
                    // case 0:
                    //     break;
                    // case 1:
                    //     for (uint64_t i = 0; i < 1 and counter && wi != 0; i++, counter--) {
                    //         wi = (w.value & word::DECODER_MASK[_case][i]) >> word::DECODER_SHIFT[_case][i];
                    //         accum += wi;
                    //     }
                    //     if (wi == 0) counter++;
                    //     break;
                    // case 2:
                    //     for (uint64_t i = 0; i < 2 and counter && wi != 0; i++, counter--) {
                    //         wi = (w.value & word::DECODER_MASK[_case][i]) >> word::DECODER_SHIFT[_case][i];
                    //         accum += wi;
                    //     }
                    //     if (wi == 0) counter++;
                    //     break;
                    // case 3:
                    //     for (uint64_t i = 0; i < 3 and counter && wi != 0; i++, counter--) {
                    //         wi = (w.value & word::DECODER_MASK[_case][i]) >> word::DECODER_SHIFT[_case][i];
                    //         accum += wi;
                    //     }
                    //     if (wi == 0) counter++;
                    //     break;
                    // case 4:
                    //     for (uint64_t i = 0; i < 4 and counter && wi != 0; i++, counter--) {
                    //         wi = (w.value & word::DECODER_MASK[_case][i]) >> word::DECODER_SHIFT[_case][i];
                    //         accum += wi;
                    //     }
                    //     if (wi == 0) counter++;
                    //     break;
                    // case 5:
                    //     for (uint64_t i = 0; i < 5 and counter && wi != 0; i++, counter--) {
                    //         wi = (w.value & word::DECODER_MASK[_case][i]) >> word::DECODER_SHIFT[_case][i];
                    //         accum += wi;
                    //     }
                    //     if (wi == 0) counter++;
                    //     break;
                    // case 7:
                    //     for (uint64_t i = 0; i < 7 and counter && wi != 0; i++, counter--) {
                    //         wi = (w.value & word::DECODER_MASK[_case][i]) >> word::DECODER_SHIFT[_case][i];
                    //         accum += wi;
                    //     }
                    //     if (wi == 0) counter++;
                    //     break;
                    // case 9:
                    //     for (uint64_t i = 0; i < 9 and counter && wi != 0; i++, counter--) {
                    //         wi = (w.value & word::DECODER_MASK[_case][i]) >> word::DECODER_SHIFT[_case][i];
                    //         accum += wi;
                    //     }
                    //     if (wi == 0) counter++;
                    //     break;
                    // case 14:
                    //     for (uint64_t i = 0; i < 14 and counter && wi != 0; i++, counter--) {
                    //         wi = (w.value & word::DECODER_MASK[_case][i]) >> word::DECODER_SHIFT[_case][i];
                    //         accum += wi;
                    //     }
                    //     if (wi == 0) counter++;
