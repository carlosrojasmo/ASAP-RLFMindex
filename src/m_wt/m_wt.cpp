...
template<class IntType>
M_WT<IntType>::M_WT (unordered_map<IntType, unsigned> freq,unordered_map<IntType, unsigned> fullfreq, unsigned n, unsigned method) {
  m_map = new std::unordered_map<IntType, unsigned>();
  switch ( method ) {
    
    case 2:
      M_WT::m_by_freq_simon_FullFreq(freq,fullfreq, n);
...
void M_WT<IntType>::m_by_freq_simon (unordered_map<IntType, unsigned> freq, unsigned n) {
  ...
  // cout << "largo m_map: " << m_map->size() << endl;
  // cout << "freq_size: " << freq_size << endl;
  construct_im(m_wavelet_tree, m_vector);
}

template<class IntType>
void M_WT<IntType>::m_by_freq_simon_FullFreq (unordered_map<IntType, unsigned> freq,unordered_map<IntType, unsigned> FullFreq, unsigned n) {
  int_vector<> m_vector(freq.size());
  ...
  vector<pair<IntType, unsigned>> byFullFreq;
  for (auto &val: byFreq) byFullFreq.push_back(make_pair(val.first,FullFreq[val.first]));
  ...
  for ( unsigned i = m_singleton_class_cnt; i < freq_size; i++ ) {
    ...
    m_conversor.push_back(byFreq[i].first);
    m_rconversor[byFreq[i].first] = i;
    m_sizes[currentL] += byFullFreq[i].second;
...
unsigned M_WT<IntType>::map (IntType c) {
  // cout<<"llego a map"<<endl;
  // cout<<"m_map["<<c<<"]: "<<(*m_map)[c]<<endl;
  return (*m_map)[c];
}
